...
CLI_FLAG(bool, clean_database, false, "Clear confgured database path");

#ifdef __linux__
FLAG(bool, lowmem, true, "Force memory allocator to run in low memory mode");
#endif
...
int startOsquery(int argc, char* argv[], std::function<void()> shutdown) {
...
#ifdef __linux__
  if (osquery::FLAGS_lowmem) {
    if (MallocExtension::instance()->SetNumericProperty(
            "tcmalloc.aggressive_memory_decommit", 1)) {
      LOG(INFO) << "tcmalloc.aggressive_memory_decommit done";
    } else {
      LOG(WARNING) << "tcmalloc.aggressive_memory_decommit failed";
    }

    if (MallocExtension::instance()->SetNumericProperty(
            "tcmalloc.max_total_thread_cache_bytes", 1)) {
      LOG(INFO)
          << "tcmalloc.max_total_thread_cache_bytes set to default minimum";
    } else {
      LOG(WARNING) << "Failed to set tcmalloc.max_total_thread_cache_bytes";
    }
  }
#endif
  ...
  if (FLAGS_install) {
    ...
    return 1;
  } else if (FLAGS_clean_database) {
    Initializer::clearOnDiskDatabase();
    return 1;
  }
