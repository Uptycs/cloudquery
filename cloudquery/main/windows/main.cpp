...
void CreateMiniDump(EXCEPTION_POINTERS* pep) {
  wchar_t szPath[MAX_PATH * 2 + 1] = {0}; // Path where to save the dump
  wchar_t szImageName[128 + 1] = {0}; // Image name of the current process
  wchar_t szTime[20 + 1] = {0}; // current time
  wchar_t szDate[20 + 1] = {0}; // current date
  std::wstring sTargetFolder(L"C:\\ProgramData\\Uptycs\\Osquery\\Log\\"),
      sImageName;
  wchar_t* imageNamePtr = NULL;
  HANDLE hFile = INVALID_HANDLE_VALUE;
  HMODULE hModule = NULL;

  // Get image name of the current process
  if (!GetModuleFileNameW(NULL, szImageName, 128)) {
    sImageName = L"Unknown";
  }
  if (!GetFullPathNameW(szImageName, _countof(szPath), szPath, &imageNamePtr)) {
    sImageName = L"Unknown";
  }
  if (imageNamePtr)
    sImageName.assign(imageNamePtr);
  imageNamePtr = NULL;

  // Get the current time : Format : HH-MM-SS
  GetTimeFormatW(
      NULL, TIME_FORCE24HOURFORMAT, NULL, L"hh'-'mm'-'ss", szTime, 20);

  // Get the current date : Format : MM-DD-YY
  GetDateFormatW(NULL, NULL, NULL, L"MM'-'dd'-'yy", szDate, 20);
...
LONG NTAPI OsqueryExceptionHandler(PEXCEPTION_POINTERS pExceptionInfo) {
  if (pExceptionInfo) {
...
int main(int argc, char* argv[]) {
  SetUnhandledExceptionFilter(
      (LPTOP_LEVEL_EXCEPTION_FILTER)&osquery::OsqueryExceptionHandler);
