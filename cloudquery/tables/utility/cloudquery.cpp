...
QueryData genCloudqueryEvents(QueryContext& context) {
  QueryData results;
  ...
  for (const auto& publisher : publishers) {
    ...
    if (pubref != nullptr) {
      ...
      if ((publisher == "auditeventpublisher") &&
        ...
        for (const auto& subscriber : subscribers) {
          ...
          if (subref != nullptr) {
            if (subref->getType() == "auditeventpublisher") {
              pubEvents += subref->numEvents();
            }
          }
        }
        r["events"] = INTEGER(pubEvents);
      } else {
  ...
  for (const auto& subscriber : subscribers) {
    ...
    if (subref != nullptr) {
      ...
      if ((isPlatform(PlatformType::TYPE_LINUX)) &
          (subref->getType() == "auditeventpublisher")) {
        r["events"] = INTEGER(subref->numEventsProcessed());
      } else {
...
QueryData genCloudqueryPacks(QueryContext& context) {
  QueryData results;
...
QueryData genCloudqueryFlags(QueryContext& context) {
  QueryData results;
...
QueryData genCloudqueryRegistry(QueryContext& context) {
  QueryData results;
  ...
  for (const auto& registry_name : rf.names()) {
    ...
    for (const auto& route : registry->getInProc()) {
      Row r;
      r["registry"] = registry_name;
      r["name"] = route.first;
      r["owner_uuid"] = INTEGER(route.second);
...
QueryData genCloudqueryExtensions(QueryContext& context) {
  QueryData results;
...
QueryData genCloudqueryInfo(QueryContext& context) {
  QueryData results;

  Row r;
  r["pid"] = INTEGER(PlatformProcess::getCurrentPid());
  r["version"] = ::osquery::cloudquery::kCQVersion;
...
QueryData genCloudquerySchedule(QueryContext& context) {
  QueryData results;
...
QueryData genCloudqueryStats(QueryContext& context) {
  QueryData results;
  Row r;

  std::uint64_t totalEventProcessed = 0;
  std::uint64_t totalPubEvents = 0;
  std::uint64_t totalAuditEventsProcessed = 0;

  r["time"] = BIGINT(getUnixTime());
  r["cloudquery_version"] = ::osquery::cloudquery::kCQVersion;
  ...
  for (const auto& subscriber : subscribers) {
    ...
    if (subref != nullptr) {
      if (isPlatform(PlatformType::TYPE_LINUX)) {
        totalAuditEventsProcessed += subref->numEventsProcessed();
      }
      totalEventProcessed += subref->numEvents();
    }
  }
  ...
  if (!isPlatform(PlatformType::TYPE_LINUX)) {
    ...
    for (const auto& publisher : publishers) {
      auto pubref = EventFactory::getEventPublisher(publisher);
      if (pubref != nullptr) {
        totalPubEvents += pubref->numEvents();
      }
    }
    // Publisher published but subscriber did not process
    r["events_dropped"] = BIGINT(totalPubEvents - totalEventProcessed);
  } else {
    totalPubEvents = totalEventProcessed;
    r["events_dropped"] = BIGINT(totalPubEvents - totalAuditEventsProcessed);
  }

  // Subscriber processed events
  r["events_processed"] = BIGINT(totalEventProcessed);

  r["uptime"] = BIGINT(getUnixTime() - Config::getStartTime());

  r["size_of_logs_forwarded"] = BIGINT(TLSRequestHelper::getForwardedEvents());

  auto& rf = RegistryFactory::get();
  if (!rf.exists("logger", rf.getActive("logger"), true)) {
    return results;
  }

  auto plugin = rf.plugin("logger", rf.getActive("logger"));
  if (plugin) {
    PluginRequest req = {{"action", "getcount"}};
    PluginResponse res;
    plugin->call(req, res);
    if (res.size() > 0) {
      r["received_logs"] = res[0].at("recieved");
      r["forwarded_logs"] = res[0].at("forwarded");
    }
  }
  results.push_back(r);
  return results;
...
bool parseRecursive(rapidjson::Value::ConstMemberIterator object,
  ...
  if (object->value.IsObject()) {
    ...
    values.insert({object->name.GetString(), strbuf.GetString()});
  } else {
    VLOG(1) << "Object not found in JSON document";
    return false;
  }
...
Status serializeObject(JSON& docObject,
                       std::unordered_map<std::string, std::string>& values) {
  // Get all the objects
  for (auto it = docObject.doc().MemberBegin();
       it != docObject.doc().MemberEnd();
       ++it) {
    if (!parseRecursive(it, values))
      continue;
  }
...
QueryData genCloudqueryConfig(QueryContext& context) {
  ...
  std::tuple<std::string, std::string> latestConfig;
  std::string time;
  std::string jsonConfig;
  Config::get().getLatestConfig(latestConfig);
  std::tie(time, jsonConfig) = latestConfig;

  r["node_name"] = "config_updated_time";
  r["node_config"] = time;
  results.push_back(r);

  stripConfigComments(jsonConfig);
  if (!doc.fromString(jsonConfig) || !doc.doc().IsObject()) {
    // Bad config is no config. So clear schedule / fim entries
    VLOG(1) << "Error parsing the config JSON";
    return results;
  }

  std::unordered_map<std::string, std::string> values;
  try {
    serializeObject(doc, values);
  } catch (...) {
    VLOG(1) << "Exception while parsing the config";
    return results;
  }

  for (auto config : values) {
    r["node_name"] = config.first;
    r["node_config"] = config.second;
    results.push_back(r);
  }
  return results;
}
