...
Status processRequest(Row& r) {
  try {
    ...
    if (r["headers"].size()) {
      std::vector<std::map<std::string, std::string>> vHeaders;
      getMapFromJsonString(r["headers"], vHeaders);
      for (auto& header : vHeaders[0])
      {
...
QueryData genCurl(QueryContext& context) {
  ...
  auto headers = context.constraints["headers"].getAll(EQUALS);
  ...
  if (headers.size() > 1) {
    LOG(WARNING) << "Can only accept a single headers json";
    return results;
  }
