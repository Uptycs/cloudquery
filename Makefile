...
ifneq ($(DISTRO_VERSION),$(DARWIN_BUILD))
	...
	@echo "-- Note: Installing and running cloudquery is supported on versions 10.9+"
endif
endif
...
build_deps: .setup
	@CLOUDQUERY_BUILD_DEPS=1 SKIP_DISTRO_MAIN=1 make deps
...
ifeq ($(CTAGS_EXISTS),)
	...
	@ctags -R ./external ./include ./cloudquery ./third-party
.PHONY: tags
...
cscope.files:
	@find \
		-E \
		./external ./include ./cloudquery ./third-party \
		-type f \
...
strip: .setup
	cd $(BUILD_DIR) && find ./cloudquery -executable -type f | xargs strip
