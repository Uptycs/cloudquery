...
function setup_brew() {
  ...
  log "refreshing local tap: cloudquery-local"

  TAPS="$DEPS/Library/Taps/"
  mkdir -p "$TAPS/cloudquery/"

  FORMULA_TAP="$TAPS/cloudquery/homebrew-cloudquery-local"
...
function brew_clear_cache() {
  if [[ ! -z "$CLOUDQUERY_CLEAR_CACHE" ]]; then
...
function brew_internal() {
  ...
  if [[ "$FORMULA" == *"cloudquery"* ]]; then
    if [[ -z "$CLOUDQUERY_BUILD_DEPS" ]]; then
  ...
  if [[ ! -z "$CLOUDQUERY_BUILD_BOTTLES" && "$FORMULA" == *"cloudquery"* ]]; then
