...
func DescribeLoadBalancersGenerate(osqCtx context.Context, queryContext table.QueryContext) ([]map[string]string, error) {
	...
	if len(utilities.ExtConfiguration.ExtConfAws.Accounts) == 0 && extaws.ShouldProcessAccount("aws_classic_loadbalancer", utilities.AwsAccountID) {
		utilities.GetLogger().WithFields(log.Fields{
			"tableName": "aws_classic_loadbalancer",
			"account":   "default",
		...
		for _, account := range utilities.ExtConfiguration.ExtConfAws.Accounts {
			if !extaws.ShouldProcessAccount("aws_classic_loadbalancer", account.ID) {
				continue
			}
			utilities.GetLogger().WithFields(log.Fields{
				"tableName": "aws_classic_loadbalancer",
...
func processRegionDescribeLoadBalancers(osqCtx context.Context, queryContext table.QueryContext, tableConfig *utilities.TableConfig, account *utilities.ExtensionConfigurationAwsAccount, region types.Region) ([]map[string]string, error) {
	...
	for {
		...
		for _, row := range table.Rows {
			if !extaws.ShouldProcessRow(osqCtx, queryContext, "aws_classic_loadbalancer", accountId, *region.RegionName, row) {
				continue
			}
...
func processAccountDescribeLoadBalancers(osqCtx context.Context, queryContext table.QueryContext, account *utilities.ExtensionConfigurationAwsAccount) ([]map[string]string, error) {
	...
	tableConfig, ok := utilities.TableConfigurationMap["aws_classic_loadbalancer"]
	if !ok {
		utilities.GetLogger().WithFields(log.Fields{
			"tableName": "aws_classic_loadbalancer",
	...
	for _, region := range regions {
		...
		if !extaws.ShouldProcessRegion("aws_classic_loadbalancer", accountId, *region.RegionName) {
			continue
		}
